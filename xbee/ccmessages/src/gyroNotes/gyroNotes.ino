...
void processNotesOnY() {
  int controlvalueY;
  int y;
  controlvalueY = GyroYBuffer.getPreviousElement(1);
  y = map(controlvalueY, 25, -25, 60, 100); 
  //MIDI.sendControlChange(16,y,midiChannel);
  if (y != previousY) {
    MIDI.sendNoteOff(previousY,127,midiChannel);
    MIDI.sendNoteOn(y,127,midiChannel);  
  }
  previousY = y;
  //delay(2);
}
...
void processCCMX() {
  ...
  MIDI.sendControlChange(15,x,midiChannel);
  //delay(2);
}
