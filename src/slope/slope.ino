...
void setCanHitBuffer()
{	
	int currentSlope = currentAccelX - AccelXBuffer.getPreviousElement(2);
	if(currentAccelX < 0){
		currentSlope = currentSlope * -1;
	}	
	int prevSlope = AccelXBuffer.getPreviousElement(2) - AccelXBuffer.getPreviousElement(3);
	if(AccelXBuffer.getPreviousElement(2) < 0){
		 prevSlope = prevSlope * -1;
	}
	if(prevSlope < 0 && currentSlope > -1 && abs(prevSlope) > 100){
		CanHitBuffer.addValue(1);
	}else{
		CanHitBuffer.addValue(0);
	}
}

void setCanHitBufferRotated()
{	
	int currentSlope = currentAccelY - AccelYBuffer.getPreviousElement(2);
	if(currentAccelY < 0){		
		currentSlope = currentSlope * -1;
	}
	int prevSlope = AccelYBuffer.getPreviousElement(2) - AccelYBuffer.getPreviousElement(3);
	if(AccelYBuffer.getPreviousElement(2) < 0){		
		 prevSlope = prevSlope * -1;
	}
	if(prevSlope < 0 && currentSlope > -1 && abs(prevSlope) > 100){
		CanHitBufferRotated.addValue(1);
	}else{
...
void setCanHit()
{
	...
	int hitters = 0;
	for(int i = 1 ; i <= canHitResolution ; i++){
		if(CanHitBuffer.getPreviousElement(i) == 1){
				hitters++;
		}
		if(hitters >= canHitDefinition){
			canHit = true;
			return;
		}
	}
...
void setCanHitRotated()
{
	...
	int hitters = 0;
	for(int i = 1 ; i <= canHitResolution ; i++){
		if(CanHitBufferRotated.getPreviousElement(i) == 1){
			hitters++;
		}
		if(hitters >= canHitDefinition){
			canHitRotated = true;
